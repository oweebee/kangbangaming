...
export default function App() {
  ...
  const [selectedGameDefaultTab, setSelectedGameDefaultTab] = useState('notes');
  ...
  useEffect(() => {
    ...
    const handlePopState = () => {
      ...
      if (selectedGame)    { setSelectedGame(null); setSelectedGameDefaultTab('notes'); return; }
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
        ...
        {displayedGame && displayedGame.type === 'custom'
          ? <TaskModal game={displayedGame} appUsers={appUsers} onPatchGame={patchGame} onSoftDeleteNote={softDeleteNote} onClose={() => { setSelectedGame(null); setSelectedGameDefaultTab('notes'); }} onEdit={() => { setEditingGame(displayedGame); setSelectedGame(null); setSelectedGameDefaultTab('notes'); }} isTaskBoard={isTaskBoard} token={token} defaultTab={selectedGameDefaultTab} currentUser={currentUser} />
          : displayedGame && <GameModal game={displayedGame} onClose={() => { setSelectedGame(null); setSelectedGameDefaultTab('notes'); }} api={API} token={token} onPatchGame={patchGame} onSoftDeleteNote={softDeleteNote} defaultTab={selectedGameDefaultTab} currentUser={currentUser} appUsers={appUsers} />
        }
  ...
  return (
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      ...
      {editingGame && <SearchModal api={API} token={token} boardGames={games} onAdd={addGame} onRemove={removeGame} onClose={() => setEditingGame(null)} customOnly={isTaskBoard} isTaskBoard={isTaskBoard} appUsers={appUsers} currentUser={currentUser} initialGame={editingGame} onSave={async g => { await updateGame(g); setEditingGame(null); }} />}
      {displayedGame && displayedGame.type === 'custom'
        ? <TaskModal game={displayedGame} appUsers={appUsers} onPatchGame={patchGame} onClose={() => { setSelectedGame(null); setSelectedGameDefaultTab('notes'); }} onEdit={() => { setEditingGame(displayedGame); setSelectedGame(null); setSelectedGameDefaultTab('notes'); }} isTaskBoard={isTaskBoard} token={token} defaultTab={selectedGameDefaultTab} currentUser={currentUser} />
        : displayedGame && <GameModal game={displayedGame} onClose={() => { setSelectedGame(null); setSelectedGameDefaultTab('notes'); }} api={API} token={token} onPatchGame={patchGame} defaultTab={selectedGameDefaultTab} currentUser={currentUser} appUsers={appUsers} />
      }
      {showAdmin && <AdminPanel token={token} currentUser={currentUser} onClose={() => setShowAdmin(false)} />}
