...
function DiscordServerIcon({ size = 22, borderColor = 'var(--surface1)' }) {
  ...
  if (err) return (
    <div style={{ width: size, height: size, borderRadius: '50%', background: '#5865f2', marginLeft: -(size * 0.25), position: 'relative', zIndex: 2, border: `2px solid ${borderColor}`, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
  ...
  return <img src={DISCORD_ICON_URL} alt="" onError={() => setErr(true)} style={{ width: size, height: size, borderRadius: '50%', marginLeft: -(size * 0.25), position: 'relative', zIndex: 2, border: `2px solid ${borderColor}`, flexShrink: 0 }} />;
}
      ...
      <div style={{ background: 'var(--surface1)', border: '1px solid var(--border)', borderRadius: 14, padding: '40px 36px', width: '100%', maxWidth: 380, boxShadow: '0 8px 40px rgba(0,0,0,.4)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 32, justifyContent: 'center' }}>
          <div style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', flexShrink: 0 }}>
            <svg viewBox="0 0 496 512" xmlns="http://www.w3.org/2000/svg" style={{ width: 32, height: 32, fill: 'var(--accent)', position: 'relative', zIndex: 1 }}>
            ...
            <DiscordServerIcon size={22} borderColor="var(--surface1)" />
          </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {info && (
            <div style={{ background: 'rgba(255,200,0,.12)', border: '1px solid rgba(255,200,0,.35)', borderRadius: 8, padding: '10px 12px', color: '#ffc800', fontSize: 13 }}>
              ⏳ {info}
            </div>
          )}
          {error && (
          ...
          <button type="submit" disabled={loading}
            style={{ marginTop: 4, padding: '11px', background: 'var(--accent)', border: 'none', borderRadius: 8, color: '#fff', fontSize: 14, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1, transition: 'opacity .15s' }}>
        ...
        <div style={{ marginTop: 20, textAlign: 'center', fontSize: 13, color: 'var(--text-muted)' }}>
          ...
          <button onClick={onGoRegister} style={{ background: 'none', border: 'none', color: 'var(--accent)', cursor: 'pointer', fontWeight: 600, fontSize: 13, padding: 0 }}>S'inscrire</button>
        </div>
