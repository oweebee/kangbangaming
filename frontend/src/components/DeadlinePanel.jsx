...
// ── Carte tâche (style board card, horizontal) ────────────────────────────────
function TaskDeadlineCard({ task, onOpen }) {
  ...
  return (
    ...
    >
      {/* Image / banner */}
      <div style={{ height: 60, background: 'var(--surface2)', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {task.header_img ? (
          <img src={task.header_img} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        ) : (
          <span style={{ fontSize: 22 }}>{tt?.emoji || task.emoji || '📋'}</span>
        )}
      </div>
      ...
      <div style={{ padding: '7px 9px', display: 'flex', flexDirection: 'column', gap: 3, flex: 1 }}>
        ...
        {info && (
          <div style={{
            marginTop: 2, fontSize: 9, fontWeight: 700, padding: '2px 6px', borderRadius: 4,
...
function Section({ cat, tasks, onOpenTask }) {
  const [collapsed, setCollapsed] = useState(false); // toutes ouvertes par défaut
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      ...
      >
        ...
        <span style={{ fontSize: 10, color: 'var(--text-muted)', transition: 'transform .15s', transform: collapsed ? 'rotate(-90deg)' : 'rotate(0deg)' }}>▾</span>
      </div>
      {!collapsed && (
  ...
  if (loading) return (
    <div style={{ marginBottom: 36 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 700, color: '#47a7f5', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Échéances</span>
      </div>
      <div style={{ color: 'var(--text-muted)', fontSize: 12 }}>Chargement…</div>
    </div>
  );

  // Pas de section si rien à afficher
